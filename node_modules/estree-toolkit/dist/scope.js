...
const identifierCrawlers = {
    ArrowFunctionExpression(key, path, state) {
        switch (key) {
            ...
            default: (0, helpers_1.assertNever)(key);
        }
    },
    ...
    ArrayExpression(listKey, path, state) {
        switch (listKey) {
            case 'elements':
                ...
                break;
            /* istanbul ignore next */
            default: (0, helpers_1.assertNever)(listKey);
        }
    },
...
const findVisiblePathsInPattern = (path, result) => {
    switch (path.node.type) {
        ...
        /* istanbul ignore next */
        default: (0, helpers_1.assertNever)(path.node);
    }
};
...
const scopePathCrawlers = {
    ...
    ForXStatement(path, state) {
        ...
        else if (is_1.is.pattern(path.node.left)) {
            registerConstantViolationFromPattern(path.get('left'), state);
        }
    },
...
class Scope {
        ...
        this.clearMemo();
        {
            for (let i = 0; i < state.references.length; i++) {
                ...
                if (binding != null) {
                    ...
                    ((_a = this.globalBindings)[bindingName] || (_a[bindingName] = new binding_1.GlobalBinding({ name: bindingName }))).addReference(path);
                }
            }
            for (let i = 0; i < state.constantViolations.length; i++) {
                ...
                if (binding != null) {
                    ...
                    ((_b = this.globalBindings)[bindingName] || (_b[bindingName] = new binding_1.GlobalBinding({ name: bindingName }))).addConstantViolation(path);
                }
            }
    ...
    registerBinding(kind, identifierPath, bindingPath) {
        ...
        this.bindings[bindingName] = new binding_1.Binding({
            kind: kind,
            name: bindingName,
            scope: this,
            identifierPath,
    ...
    generateUidIdentifier(name) {
        return builders_1.builders.identifier(this.generateUid(name));
    }
    generateDeclaredUidIdentifier(name) {
        ...
        if (this.priv.declaration == null) {
            ...
            switch (this.path.type) {
                case 'ArrowFunctionExpression':
                    {
                        ...
                        if (body.type === 'BlockStatement') {
                            ...
                            block = body.replaceWith(builders_1.builders.blockStatement([builders_1.builders.returnStatement(bodyNode)]));
                        }
                    }
                    break;
                case 'Program':
                ...
                case 'WhileStatement':
                    {
                        ...
                        if (body.type === 'BlockStatement') {
                            ...
                            const bodyNode = Object.assign({}, body.node);
                            block = body.replaceWith(builders_1.builders.blockStatement([bodyNode]));
                        }
                    }
                    break;
                case 'CatchClause':
                ...
                case null: break;
                /* istanbul ignore next */
                default: (0, helpers_1.assertNever)(this.path.type);
            }
            const declarationNode = builders_1.builders.variableDeclaration('var', [builders_1.builders.variableDeclarator(this.generateUidIdentifier(name))]);
            ...
            [declaratorPath] = this.priv.declaration.pushContainer('declarations', [builders_1.builders.variableDeclarator(this.generateUidIdentifier(name))]);
        }
    ...
    renameConsideringParent(path, newName) {
        ...
        if (parent.type === 'Property' &&
            ...
            ((_d = (_c = path.parentPath) === null || _c === void 0 ? void 0 : _c.parent) === null || _d === void 0 ? void 0 : _d.type) === 'Property' &&
            ((_f = (_e = path.parentPath.parentPath) === null || _e === void 0 ? void 0 : _e.parent) === null || _f === void 0 ? void 0 : _f.type) === 'ObjectPattern') {
            const property = path.parentPath.parent;
            parent.left = builders_1.builders.identifier(newName);
            property.shorthand = parent.left.name === property.key.name;
        }
        else {
